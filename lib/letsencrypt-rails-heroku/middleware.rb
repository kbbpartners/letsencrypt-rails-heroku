module Letsencrypt
  class Middleware
    ...
    def _call(env)
      ...
      challenge_filename = "/#{Letsencrypt.challenge_filename}"
      challenge_file_content = "#{Letsencrypt.challenge_file_content}"
      matching_paths = current_path == challenge_filename

      if Letsencrypt.challenge_configured? && matching_paths
        return [200, {"Content-Type" => "text/plain"}, [Letsencrypt.challenge_file_content]]
      else
